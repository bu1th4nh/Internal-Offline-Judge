...
void Preps()
{
    ...
    srt = comp = vect;
    sort(whole_1(srt));
    FOR(i, 1, n) comp[i] = lower_bound(whole_1(srt), vect[i]) - srt.begin();

    FOR(i, 1, n)
    {
        dp[i] = ft.query(comp[i]) + 1;
        ft.update(comp[i], dp[i]);
    }
}
void Solve()
{
    int u, v, id;
    res = vvi(*max_element(whole_1(dp)));
    FOR(i, 1, n)
    {
        tie(u, v, id) = a[i];
        res[dp[i]-1].push_back(id);
    }

    print(res.size()); pc(el);
    for(auto &x: res) reverse(whole(x));
    for(auto x: res)
    {
        print(x.size()), pc(sp);
        for(auto y: x) print(y), pc(sp);
        pc(el);
    }
}
