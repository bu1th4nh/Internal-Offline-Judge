...
//Cliques
void maximalClique(bs candidate, bs chosen)
{
    if(!candidate.any()) { maximize(res, chosen.count()); return; }
    if(candidate.count() + chosen.count() <= res) return;

    int u = candidate._Find_first();

    bs A;
    candidate[u] = 0;
    maximalClique(candidate, chosen);
    candidate[u] = 1;


    chosen[u] = 1;
    for(int v: adj[u]) A[v] = 1;
    maximalClique(candidate & A, chosen);
    chosen[u] = 0;
}
