...
/*
        #Picks-theorem #diophantine-function

        Picks's theorem:

            Area = (Nr. of points inside the polygon) + (Nr. of points on the boundary of polygon) / 2 - 1.

        ==> Answer = (Nr. of points inside the polygon) + (Nr. of points on the boundary of polygon)
                   = Area * 2 - (Nr. of points inside the polygon) + 2
*/
//=====================================
...
void Solve()
{
    ...
    FOR(i, 0, n-1)
    {
        pt t = a[i] - a[i+1];
        S += calc(a[i], a[i+1]);
        B += __gcd(__abs(t.x), __abs(t.y));
    }

    //cerr << __abs(S) << sp << B << el;
    cout << (__abs(S) + B + 2) / 2;
}
