...
//Process
void Parsing(const vi &s, vi &F)
{
    msint st;
    FOR(i, 1, n)
    {
        st.insert(s[i]);
        auto ptr = st.lower_bound(s[i]); ptr++;
...
void Solve()
{
    FOR(i, 1, n) a[i] -= constr;
    Parsing(a, start);
    FOR(i, 1, n) a[i] += constr;
    reverse(whole_1(a));
    FOR(i, 1, n) a[i] = -a[i];
    Parsing(a, finish);
    reverse(whole_1(finish));

    int res = 0;
    FOR(i, 1, n) res = max(res, start[i] + finish[i] - 1);
    cout << res;
}
