...
//=====================================
//Constants
const int nextx[2] = {0, 1};
const int nexty[2] = {1, 0};
...
bool Verify(string s)
{
    ...
    while(i < (int)s.size())
    {
        if(s[i] == sample[j]) ++i, ++j;
        if(j == sz) return 1;
        else if(i < (int)s.size() && s[i] != sample[j])
        {
            if(j) j = lps[j-1];
            else ++i;
        }
...
void DFS(ii u)
{
    ...
    //cerr << u.first << sp << u.second << el;

    FOR(i, 0, 1)
    {
        ii v = ii(u.first + nextx[i], u.second + nexty[i]);
...
void computeLPS()
{
    ...
    while(i < sz)
    {
        if(sample[i] == sample[len]) lps[i++] = ++len;
        else
        {
