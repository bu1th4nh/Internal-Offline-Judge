...
void Enter()
{
    ...
    {
        read(x);
        vect.push_back(ii(x, 1));
    }
    FOR(i, 1, m)
    {
        read(x);
        vect.push_back(ii(x, 2));
    }
}
...
void Solve()
{
    ...
    sort(whole(vect));
    FORl(i, 1, vect.size()) if(vect[i].se != vect[i-1].se)
    {
        int tmp = __abs(vect[i].fi - vect[i-1].fi);
        if(res1 > tmp)
        {
            res1 = tmp;
            res2 = 1;
        }
        else if(res1 == tmp) ++res2;
    }

    cout << res1 + __abs(c1 - c2) << sp << res2 << el;
}
