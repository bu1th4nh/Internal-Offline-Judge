...
void Enter()
{
	...
	num = low = par = nDest = nChild = vi(n+1, 0);
	isCut = vb(n+1, 0);
	adj = vvi(n+1);
	timeStamp = 0;
...
void DFS(int u, int p)
{
    ...
    for(int v: adj[u]) if(v != p)
    {
        ...
        else
        {
            ...
            if(p && low[v] >= num[u]) isCut[u] = 1;
        }
    }

    if(!p && nChild[u] >= 2) isCut[u] = 1;
    nDest[p] += nDest[u];
}
...
void Solve()
{
    ...
    FOR(i, 1, n)
    {
        ...
        else
        {
            ...
            for(int v: adj[i]) if(par[v] == i && low[v] >= num[i])
            {
                remain -= nDest[v];
                res -= __sqr(nDest[v] * 1LL);
            }
            res -= __sqr(remain);
