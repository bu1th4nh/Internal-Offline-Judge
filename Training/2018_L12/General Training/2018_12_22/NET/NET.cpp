...
void Enter()
{
    ...
    timeStamp = 0;
    adj = vvi(n+1);
    a = b = num = low = vi(n+1, 0);

    FOR(i, 1, na) {scan(u); ++a[u];}
    FOR(i, 1, nb) {scan(u); ++b[u];}
...
bool check(ii edg)
{
    int u = edg.fi, v = edg.se;

    if(a[u] == 0 || b[u] == 0) return 1;
    if(a[v] == 0 || b[v] == 0) return 1;
    if(a[1] == a[u] && a[u] == a[v]) return 1;
    if(b[1] == b[u] && b[u] == b[v]) return 1;
...
{
    DFS(1, 0);
    for(ii x: bridge) res += (int)check(x);
    cout << res << el;
}
