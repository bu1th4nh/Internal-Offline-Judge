...
void Subtask1()
{
    ...
    FORl(state, 1, 1 << (n))
    {
        vi smp, picked(n, 0);
        bool ok = 1;
        FORl(i, 0, n) if(getBit(state, i)) smp.push_back(i), picked[i] = 1;

        FORl(i, 0, smp.size())
        {
            FORl(j, i+1, smp.size()) if(a[smp[i]][smp[j]] || a[smp[j]][smp[i]])
            {
                ok = 0;
                break;
            }
        ...
        if(ok)
        {
            for(int v: smp) for(int x: adj[v+1]) picked[x-1] = 1;
            if(accumulate(whole(picked), 0) != n)
            {
                ok = 0;
            }
        }

        if(ok && cnt < (int)smp.size())
        {
            res = smp;
            cnt = smp.size();
        }
