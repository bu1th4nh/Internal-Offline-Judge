...
vi topoOrder, idOrder, dp, nPath;
int n, m, s, t, timeStamp;
vvii adj;
vb avail;
...
void Enter()
{
    ...
    dp = vi(n+1, -inf);
    avail = vb(n+1, 1);
    topoOrder = idOrder = nPath = vi(n+1, 0LL);
...
{
    avail[u] = 0;
    for(ii v: adj[u]) if(avail[v.se]) topoDFS(v.se);
