...
void Generate_input()
{
    ...
    int n = rand_int(1000, 1);
    test_inp << n << el;
    FOR(i, 1, n) test_inp << rand_int(1000000000, 700000000) << sp;
}
void Generate_answer()
{
    ...
    FOR(i, 1, n)
    {
        ...
        {
            ll mxe = a[i], mne = a[i];
            FOR(k, i, j) mxe = max(mxe, (ll)a[k]);
            FOR(k, i, j) mne = min(mne, (ll)a[k]);
...
void Checker()
{
    ...
    FOR(iTest, 1, T)
    {
        ...
        int ec = system("fc "task".out "task".ans"); cout << "FC exit code " << ec << el;
