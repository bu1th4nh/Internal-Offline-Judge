...
void Preps()
{
    ...
    r_fact[n] = powmod(fact[n], mod-2);
    FORb(i, n-1, 1) r_fact[i] = (r_fact[i+1] * 1LL * (i+1)) % mod;

    #ifdef DEBUG
        FOR(i, 1, n) cerr << fact[i] << sp; cerr << el;
        FOR(i, 1, n) cerr << r_fact[i] << sp; cerr << el;
    #endif // DEBUG
}
...
void Solve()
{
    ll ckn = ((fact[n] * r_fact[n-k]) % mod * r_fact[k]) % mod;
    dp[1] = 0LL;
    dp[2] = 1LL;

    FOR(i, 3, n-k+1)
    {
        dp[i] = 1LL * ((n-1) * (dp[i-2] + dp[i-1])) % mod;
    }
    ll res = 1LL * (ckn * dp[n-k]) % mod;
