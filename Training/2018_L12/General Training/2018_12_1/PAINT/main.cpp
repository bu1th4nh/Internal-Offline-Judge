...
void Solve()
{
    ...
    FOR(i, 1, n)
    {
        ...
        if(!q.empty() && q.front() + x <= i) q.pop_front();

        q.push_back(i);
        if(i >= x) mini[i] = a[q.front()];
    }
    FORb(i, x-1, 0) mini[i] = mini[i+1];


//    FOR(i, 1, n) cerr << mini[i] << sp; cerr << el;
    ...
    FOR(iQ, 1, n)
    {
        ...
        while(true)
        {
            ...
            while(!q.empty() && mini[i] >= mini[q.back()]) q.pop_back();
            q.push_back(i);
