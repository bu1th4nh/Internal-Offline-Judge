...
struct EulerPath
{
    ...
    void find_path(int u)
    {
        while(adj[u].size() > 0)
        {
            int v = adj[u].front().to;
            adj[v].erase(adj[u].front().rev);
            adj[u].pop_front();
    ...
    void add_edge(int u, int v)
    {
        ...
        auto itu = adj[u].begin();
        adj[v].push_front(Edge(u));
        auto itv = adj[v].begin();
