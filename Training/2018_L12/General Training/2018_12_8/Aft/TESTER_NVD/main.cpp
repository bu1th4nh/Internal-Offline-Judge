...
void Generate_input()
{
    ...
    FOR(i, 1, n)
    {
        ll a = rand_int(1, 1000000000000LL);
        ll b = rand_int(1, 1000000000000LL);
        assert(a > 0 && b > 0);
        if(a > b) swap(a, b);
        test_inp << a << sp << b << el;
    }
...
void Checker()
{
    ...
    FOR(iTest, 1, T)
    {
        ...
        system(task);
        timeEnd   = chrono::steady_clock::now();
        long double t = chrono::duration<double>(timeEnd - timeStart).count();

        int ec2 = 1 - system("fc "task".out "task".ans");
        int ec = int(t <= 1.000000000000000000000);

        cout << "Sample program completed with elapsed time: " << t << " second(s) with verdict ";

        if(ec2 == 0) puts("WA");
        else puts((ec == 1) ? "Accepted" : "TLE");
        score += (ec && ec2);
    }
