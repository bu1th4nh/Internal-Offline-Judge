...
void Dijkstra()
{
    ...
    pq.push(make_tuple(0LL, initial_height*1LL, 1));
    dist[1] = 0LL;

    while(!pq.empty())
    {
        tie(du, hu, u) = pq.top(); pq.pop(); //hu = hu;
        if(du != dist[u]) continue;
        if(u == n) minimize(res, du + (height[n] - hu));
        ...
        for(ii x: adj[u])
        {
            ...
            if(hu - t < 0)                      //Case 1: hu - t < 0
            {
                if(height[u] < t) continue;
                else hv = 0, uv += t - hu;
            }
            else if(hu - t > height[v])         //Case 2: hu - t > height[v]
            {
                hv = height[v];
                uv += hu - t - height[v];
            }
            ...
            if(dist[v] > du + uv) pq.push(iii((dist[v] = du + uv) , hv , v));
        }
    }
}
