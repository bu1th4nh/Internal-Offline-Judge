...
void BFS()
{
    ...
    FOR(i, 1, m) pq.push(iii(1, i, 1)), dist[i][1] = 1;
    ...
    while(!pq.empty())
    {
        ...
        FORl(i, 0, 4)
        {
            ...
            if(check(x, y))
            {
                if(dist[x][y] > dist[cx][cy] + int(a[cx][cy] != a[x][y])) {
                    pq.push(iii( dist[x][y] = dist[cx][cy] + int(a[cx][cy] != a[x][y]) , x, y));
                }
            }
        }
...
//        FOR(i, 1, m)
//        {
//            FOR(j, 1, n) if(dist[i][j] >= inf) cerr << "-" << " \n"[j == n];
//            else cerr << dist[i][j] << " \n"[j == n];
//        }
//        getchar();
    }
...
void Solve()
{
    int res = inf;
    BFS();
    FOR(i, 1, m) minimize(res, dist[i][n]);
    print(res);
}
