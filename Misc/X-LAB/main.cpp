...
int main()
{
    ...
    FOR(k, 1, n)
    {
        FOR(i, 1, n)
        {
            FOR(j, 1, n) b[i][j] = min(b[i][j], b[i][k]+b[k][j]);
        }

        FOR(i, 1, n)
        {
            FOR(j, 1, n) c[k][i] = min(c[k][i], c[k][j]+b[j][i]);
        }

        cerr << "+++++++++++++++++++++++++++++++\n";
        FOR(i, 1, n)
        {
            FOR(j, 1, n) if(b[i][j] >= inf) cerr << '-' << sp;
            else cerr << b[i][j] << sp;
            cerr << el;
        }
        cerr << el;
        FOR(i, 1, n)
        {
            FOR(j, 1, n) if(c[i][j] >= inf) cerr << '-' << sp;
            else cerr << c[i][j] << sp;
            cerr << el;
        }
        cerr << "+++++++++++++++++++++++++++++++\n";
